...
public class ClassDao {
    ...
    public PageModel<ClassSearchResponse> allClass(ClassSearchRequest request, int pageNo, int pageSize) {
        ...
        List<ClassSearchResponse> list = jdbcTemplate.query(ClassSqlMapping.SELECT_ADMINCLASS_LIST + searchSql + limitSql, new RowMapper<ClassSearchResponse>() {
            ...
            public ClassSearchResponse mapRow(ResultSet rs, int rows) throws SQLException {
                ...
                response.setAdminClassId(rs.getInt("classId"));              response.setAdminClassName(rs.getString("className"));
    ...
    public Boolean studentDelete(Integer studentId) throws Exception {
        /*find userId by studentId*/
        List<Integer> userIdList = jdbcTemplate.query(ClassSqlMapping.FIND_USERID_BY_STUDENTID, 
                new RowMapper<Integer>() {
                    @Override
                    public Integer mapRow(ResultSet rs, int rows) throws SQLException {
        ...
        if (userIdList.size() != 0 ) {
            systemDao.deleteUser(userIdList.get(0));
        }
        /*delete student by studentId*/
        Integer delStudent = jdbcTemplate.update(ClassSqlMapping.DELETE_STUDENT, studentId);
        return true;
    }
    ...
    public Boolean classDelete(Integer classId) throws Exception {
        /*find studentId by classId*/
        List<Integer> studentIdList = jdbcTemplate.query(ClassSqlMapping.FIND_STUDNETSID_BY_CLASSID,
                new RowMapper<Integer>() {
                    @Override
                    public Integer mapRow(ResultSet rs, int rows) throws SQLException {
        ...
        /*delete notice by classId*/
        Integer delNotice = jdbcTemplate.update(ClassSqlMapping.DELETE_NOTICE_BY_CLASSID, classId);
        
        /*delete administrator by classId*/
        Integer delAdministrator = jdbcTemplate.update(ClassSqlMapping.DELETE_ADMIN_BY_CLASSID, classId);
        
        /*delete class by classId*/
        Integer delClass = jdbcTemplate.update(ClassSqlMapping.DELETE_CLASS, classId);
        return true;
    }
}
